...
--We check the distinct months in yeach year of the whole data from 2003-2005
--Observation: 2005 has only data of first 5 months
...
--Revenue accross the 3 years based on the productline
select PRODUCTLINE, sum(sales) Revenue
from [dbo].[sales_data_sample]
group by PRODUCTLINE
order by 2 desc

--Revenue accross the years:
select YEAR_ID, sum(sales) Revenue
from [dbo].[sales_data_sample]
group by YEAR_ID
order by 2 desc

--Revenue according the dealsize:
...
--Sorting by the productline for 2004
...
--Sorting by the productline for 2003
select  MONTH_ID, PRODUCTLINE, sum(sales) Revenue, count(ORDERNUMBER)
from [PortfolioDB].[dbo].[sales_data_sample]
